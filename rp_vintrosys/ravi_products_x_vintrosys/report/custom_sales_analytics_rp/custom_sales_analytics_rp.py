...
class Analytics(object):
	...
	def get_groups(self):
		...
		elif self.filters.tree_type == "Customer Group":
		...
		for d in self.group_entries:
			parent_name = d.parent if d.parent else None

			# Root group
			if not parent_name:
				self.depth_map[d.name] = 0
			else:
				# If parent not set yet, assume depth -1 (so child becomes 0)
				parent_depth = self.depth_map.get(parent_name, -1)
				self.depth_map[d.name] = parent_depth + 1
